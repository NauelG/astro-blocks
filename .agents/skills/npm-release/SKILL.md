...
- [ ] **Run `npm version`** — `npm version <major|minor|patch|X.Y.Z> -m "chore(release): %s"`.
  - `preversion` runs `npm test` (build + unit tests) — aborts on failure.
  - Bumps `package.json#version`.
  - `version` regenerates `img/dashboard.*`, `img/page_editor.*`, `img/media-library.png`, `img/image-picker.png` via `screenshots:readme` and `screenshots:media`, then stages `img/`.
  - Creates the release commit (package.json + img/) and the `vX.Y.Z` tag automatically.
  - **Do NOT create the tag manually** — npm already created it.
- [ ] **Push** — `git push --follow-tags`. This triggers `release-tag.yml`.
- [ ] **Workflow passes** — confirm `validate_release` and `publish_npm` jobs both succeed.
- [ ] **Dist-tag confirmed** — `npm view @astroblocks/astro-blocks dist-tags`; verify `latest` (and `alpha` for pre-releases) point to the new version.
...
- **Version bump mechanism**: use `npm version <type> -m "chore(release): %s"` — do NOT hand-edit `package.json` version or create the tag manually. npm's `preversion` hook runs tests, the `version` hook regenerates all screenshots and stages `img/`, then npm creates the release commit and tag.
- **Screenshots are automatic**: `img/dashboard.*`, `img/page_editor.*`, `img/media-library.png`, `img/image-picker.png` are regenerated by the `version` hook on every release. Do not include them as a manual checklist item.
- **Clean tree required**: commit the CHANGELOG entry before running `npm version`; npm aborts if the working tree is dirty.
