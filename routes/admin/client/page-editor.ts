  ...
  function renderPagesTable(): void {
    ...
    pagesTbody.innerHTML = list
      .map((page) => {
        ...
        return (
          '<tr>' +
          `<td class="cms-table-actions"><button type="button" class="cms-table-btn-edit cms-page-edit" data-id="${escapeAttr(page.id)}" aria-label="${ct('common.edit')}">${pencilIconSvg}</button></td>` +
          ...
          `<td class="cms-table-actions-delete"><button type="button" class="cms-table-btn-delete cms-page-delete" data-id="${escapeAttr(page.id)}" aria-label="${ct('common.delete')}">${trashIconSvg}</button></td>` +
          '</tr>'
        );
      })
      .join('');
