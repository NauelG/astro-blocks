...
function resolveInstalledPkgDir(): string {
  ...
  if (typeof import.meta.resolve === 'function') {
    ...
    process.stderr.write(
      '[astro-blocks] Node < 18.19 detected, using CommonJS resolver fallback.\n',
    );
...
async function detectTargetFile(
  cwd: string,
  flags: Flags,
): Promise<{ targetPath: string; mustCreate: boolean }> {
  if (flags.file) {
  ...
  if (useCopy || flags._resolverOverride !== undefined) {
    ...
    if (flags.dryRun) {
      process.stdout.write(
        `[astro-blocks] --dry-run (${modeLabel}): would append to ${fileName}:\n${appendContent}\n`,
      );
      return;
    }
  } else {
    ...
    if (flags.dryRun) {
      process.stdout.write(
        `[astro-blocks] --dry-run: would append to ${fileName}:\n${appendContent}\n`,
      );
      return;
    }
  }
