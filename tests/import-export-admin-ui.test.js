...
test('E-1: en catalog has nav.importExport key', async () => {
  ...
  assert.ok(typeof catalogs.en['nav.importExport'] === 'string' && catalogs.en['nav.importExport'].length > 0,
...
test('E-1: es catalog has nav.importExport key', async () => {
  ...
  assert.ok(typeof catalogs.es['nav.importExport'] === 'string' && catalogs.es['nav.importExport'].length > 0,
...
test('E-1: es catalog has all importExport.* keys (parity with en)', async () => {
  const { catalogs } = await import(`${DIST_DIR}/routes/admin/i18n/catalogs.js`);
  const enKeys = Object.keys(catalogs.en).filter((k) => k.startsWith('importExport.') || k === 'nav.importExport');
...
test('E-2: import-export.astro declares prerender = false', () => {
  ...
  assert.match(src, /export\s+const\s+prerender\s*=\s*false/, 'page must have export const prerender = false');
});
...
test('E-2: import-export.astro has <label for> associated with the file input (no label-less input)', () => {
  ...
  assert.match(src, /for="ie-import-file"/, 'file input must have an explicit label[for="ie-import-file"]');
});
...
test('E-2: import-export.astro define:vars bridge is NOT empty (contains ieI18n assignment)', () => {
  ...
  assert.match(src, /__cmsImportExportI18n/, 'define:vars bridge must assign window.__cmsImportExportI18n');
});
...
test('E-2: import-export.astro define:vars bridge is NOT an empty script tag', () => {
  ...
  assert.ok(!emptyBridgeRe.test(src), 'import-export.astro must NOT have an empty define:vars bridge');
});
...
test('E-3: client module dist is compiled (dist/routes/admin/client/import-export-editor.js exists)', () => {
  ...
  assert.ok(fs.existsSync(dist), 'compiled dist must exist at dist/routes/admin/client/import-export-editor.js');
});

test('E-3: client module uses fetch with Authorization header for export (no bare anchor URL auth)', () => {
  const src = fs.readFileSync(path.join(ADMIN_DIR, 'client', 'import-export-editor.ts'), 'utf8');
  assert.match(src, /Authorization.*Bearer/, 'export must use Authorization: Bearer header, not bare anchor navigation');
});
...
test('E-3: client module uses fetch with Authorization header for import POST', () => {
  ...
  assert.match(src, /Content-Type.*application\/zip/, 'import POST must use Content-Type: application/zip');
});
...
test('E-3: client module does not use innerHTML with untrusted content (uses safe DOM methods)', () => {
  ...
  assert.match(src, /createElement\('li'\)/, 'manifest units must be rendered via createElement, not innerHTML');
...
test('E-3: client module uses window.__cmsImportExportI18n for i18n (bridge)', () => {
  const src = fs.readFileSync(path.join(ADMIN_DIR, 'client', 'import-export-editor.ts'), 'utf8');
  assert.match(src, /__cmsImportExportI18n/, 'client must read from window.__cmsImportExportI18n bridge');
});
...
test('E-4: layout.astro has /cms/import-export nav link', () => {
  const src = fs.readFileSync(path.join(ADMIN_DIR, 'layout.astro'), 'utf8');
  assert.match(src, /href="\/cms\/import-export"/, 'layout nav must have a link to /cms/import-export');
});
...
test('E-4: /cms/import-export nav link has data-cms-owner-only attribute', () => {
  ...
  const linkPattern = /href="\/cms\/import-export"[^>]*data-cms-owner-only|data-cms-owner-only[^>]*href="\/cms\/import-export"/;
