...
test('C5: fileDownloadUrl importable from dist/utils/file-value.js', async () => {
  ...
  assert.ok(
    typeof mod.mediaEntryToFileValue === 'function',
    'mediaEntryToFileValue must be a function',
  );
});
...
test('C5: package root re-exports DEFAULT_ALLOWED_FILE_TYPES', async () => {
  ...
  assert.ok(
    ...
    'DEFAULT_ALLOWED_FILE_TYPES must be re-exported from package root',
  );
  assert.ok(Array.isArray(mod.DEFAULT_ALLOWED_FILE_TYPES), 'must be an array');
  assert.ok(
    mod.DEFAULT_ALLOWED_FILE_TYPES.includes('application/pdf'),
    'must include application/pdf',
  );
});
